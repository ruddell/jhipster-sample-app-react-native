...
test('attempt retrieving a single location', () => {
  ...
  expect(state.location).toEqual({ id: undefined })
})
...
test('attempt retrieving a list of location', () => {
  ...
  expect(state.locationList).toEqual([])
})
...
test('success retrieving a list of location', () => {
  const state = reducer(
    INITIAL_STATE,
    Actions.locationAllSuccess([{ id: 1 }, { id: 2 }]),
  )

  expect(state.fetchingAll).toBe(false)
  expect(state.errorAll).toBe(null)
  expect(state.locationList).toEqual([{ id: 1 }, { id: 2 }])
})
...
test('resetting state for location', () => {
  const state = reducer({ ...INITIAL_STATE, deleting: true }, Actions.locationReset())
