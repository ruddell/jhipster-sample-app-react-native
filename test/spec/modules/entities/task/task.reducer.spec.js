...
test('attempt retrieving a single task', () => {
  ...
  expect(state.task).toEqual({ id: undefined })
})
...
test('success retrieving a list of task', () => {
  const state = reducer(
    INITIAL_STATE,
    Actions.taskAllSuccess([{ id: 1 }, { id: 2 }]),
  )

  expect(state.fetchingAll).toBe(false)
  expect(state.errorAll).toBe(null)
  expect(state.taskList).toEqual([{ id: 1 }, { id: 2 }])
})
...
test('resetting state for task', () => {
  const state = reducer({ ...INITIAL_STATE, deleting: true }, Actions.taskReset())
