...
test('attempt retrieving a single region', () => {
  ...
  expect(state.region).toEqual({ id: undefined })
})
...
test('attempt retrieving a list of region', () => {
  ...
  expect(state.regionList).toEqual([])
})
...
test('success retrieving a list of region', () => {
  const state = reducer(
    INITIAL_STATE,
    Actions.regionAllSuccess([{ id: 1 }, { id: 2 }]),
  )

  expect(state.fetchingAll).toBe(false)
  expect(state.errorAll).toBe(null)
  expect(state.regionList).toEqual([{ id: 1 }, { id: 2 }])
})
...
test('resetting state for region', () => {
  const state = reducer({ ...INITIAL_STATE, deleting: true }, Actions.regionReset())
