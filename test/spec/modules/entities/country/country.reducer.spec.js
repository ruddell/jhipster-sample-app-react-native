...
test('attempt retrieving a single country', () => {
  ...
  expect(state.country).toEqual({ id: undefined })
})
...
test('attempt retrieving a list of country', () => {
  ...
  expect(state.countryList).toEqual([])
})
...
test('success retrieving a list of country', () => {
  const state = reducer(
    INITIAL_STATE,
    Actions.countryAllSuccess([{ id: 1 }, { id: 2 }]),
  )

  expect(state.fetchingAll).toBe(false)
  expect(state.errorAll).toBe(null)
  expect(state.countryList).toEqual([{ id: 1 }, { id: 2 }])
})
...
test('resetting state for country', () => {
  const state = reducer({ ...INITIAL_STATE, deleting: true }, Actions.countryReset())
