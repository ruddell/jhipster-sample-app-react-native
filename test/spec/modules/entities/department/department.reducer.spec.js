...
test('attempt retrieving a single department', () => {
  ...
  expect(state.department).toEqual({ id: undefined })
})
...
test('attempt retrieving a list of department', () => {
  ...
  expect(state.departmentList).toEqual([])
})
...
test('success retrieving a list of department', () => {
  const state = reducer(
    INITIAL_STATE,
    Actions.departmentAllSuccess([{ id: 1 }, { id: 2 }]),
  )

  expect(state.fetchingAll).toBe(false)
  expect(state.errorAll).toBe(null)
  expect(state.departmentList).toEqual([{ id: 1 }, { id: 2 }])
})
...
test('resetting state for department', () => {
  const state = reducer({ ...INITIAL_STATE, deleting: true }, Actions.departmentReset())
